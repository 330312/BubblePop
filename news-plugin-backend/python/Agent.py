...
agent_prompt = {
    ...
    "A": {
        ...
        "goal": """
                ...
                【强约束】尽量覆盖不同媒体/机构来源：
                - 以 url 的网站域名判断“网站来源”；若 url 缺失可用 sourceName 代替。
                - timeline 至少覆盖 3 个不同网站来源，且除非输入里确实没有其他来源，否则不要把同一网站塞满。
                - 同一网站来源最多出现2条，超过则合并为更综合的一条或替换为其它网站来源。
...
class ReActTrinityAnalyzer:
    ...
    def _max_chars_for_step(self, agent_key: str) -> int:
        ...
        if agent_key == "C":
            return _to_int(os.getenv("AGENT_MAX_CHARS_RELATED_EVENTS", "2000"), 2000)
                ...
                if isinstance(data, dict):
                    ...
                    if isinstance(data.get("data"), dict):
                        ...
                        if isinstance(nested, dict):
                            return nested
                return data
            return {"code": 500, "msg": "未在模型输出中找到有效的 Action JSON 结构", "data": None}
        except Exception as e:
            return {"code": 500, "msg": "error", "data": f"JSON 解析错误: {str(e)}"}
